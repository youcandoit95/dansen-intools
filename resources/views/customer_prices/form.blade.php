...
<form action="{{ isset($price) ? route('customer-prices.update', $price->id) : route('customer-prices.store') }}"
    ...
    $agentSelected = old('sales_agent_id', $price->sales_agent_id ?? '');
    @endphp
    ...
    <div class="mb-4">
        ...
        <select name="customer_id"
            ...
            data-selected="{{ $customerSelected }}"
            required>
            <option value="">-- Pilih Customer --</option>
            @foreach($customers as $cust)
            <option value="{{ $cust->id }}" {{ $customerSelected == $cust->id ? 'selected' : '' }}>{{ $cust->nama }}</option>
            @endforeach
        </select>
    </div>
    ...
    <!-- Sales Agent -->
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-1">Sales Agent (opsional)</label>
        <select name="sales_agent_id"
            ...
            <option value="{{ $agent->id }}" {{ $agentSelected == $agent->id ? 'selected' : '' }}>{{ $agent->nama }}</option>
            @endforeach
        </select>
    </div>
    ...
    <!-- Info Harga -->
    <div id="hargaInfo" class="mb-4 hidden border rounded p-4 bg-gray-50 text-sm"></div>
    ...
    <!-- Komisi Sales -->
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-1">Komisi untuk Sales Agent (Rp)</label>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const salesAgentSelect = document.getElementById('sales_agent_id');
        if (salesAgentSelect) new TomSelect('#sales_agent_id');
        const customerSelect = document.querySelector('[name="customer_id"]');
        if (customerSelect) new TomSelect('[name="customer_id"]');
        ...
        salesAgentSelect?.addEventListener('change', function() {
            komisiInput.disabled = !this.value;
            if (!this.value) komisiInput.value = '';
        });
